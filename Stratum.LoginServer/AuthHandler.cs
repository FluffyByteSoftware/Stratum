...
using Stratum.Shared.Networking.Packets.Auth;
using Stratum.SystemTools.Accounts;
using Stratum.SystemTools.Logger;
using Stratum.SystemTools.Security;
using System;
    ...
    string udpEndpoint)
{
    ...
    public async ValueTask OnAuthByKey(TcpConnection conn, AuthByKeyPacket packet)
    {
        if(!accounts.TryGet(packet.AccountId, out var record))
        {
            ...
            conn.RequestDisconnect(Shared.Networking.SecureDisconnectReason
        ...
        TcpConnection conn, AuthByPasswordPacket packet)
    {
        ...
        if(!PasswordHasher.Verify(packet.Password, record.PasswordHash))
        {
            ...
            conn.RequestDisconnect(
                Shared.Networking.SecureDisconnectReason.InvalidUserCredentials);
            return;
        }
        ...
        TcpConnection conn, AuthResponsePacket response)
    {
        ...
        conn.RequestDisconnect(Shared.Networking.SecureDisconnectReason.None);
    }
