...
All numbers are **little-endian** (lowest byte first).  That is what C#'s `BinaryWriter` and `BinaryReader` do already for numbers -- but not for strings.  `BinaryWriter.Write(string)` puts its own 1 to 5 byte count in front, not a `u32`, and the server refuses what it sends.  Write a string's four bytes of length yourself (`BinaryPrimitives` does it), then the UTF-8.
...
The WorldTicket hands the client a token and a UDP port.  The client takes both to the server's UDP side, and from then on the game goes over UDP, and the TCP connection stays open for chat.  The two are kept apart on purpose: if the game side falls over, chat keeps working, and the other way round.

UDP has no TLS and no frame.  It keeps every packet whole and apart from the next, so a UDP packet is only the type and the payload:

```text
[type: u8][payload]
```

Numbers and strings are the same as on TCP.  The most the server takes is 1200 bytes a packet.  Anything much bigger risks getting split up along the way, and a lost piece loses the whole thing.

Getting connected:

1. The client sends **Connect**: its version, then the token from the WorldTicket.
2. The server checks the version first.  It has to match one on the server's list exactly (`expected_client_version.txt`, at the repo root, built into the server).  If it doesn't, the answer is **ConnectResult** 2, "Outdated Client Failure", and the token isn't even looked at.
3. Then the token.  A token the server didn't hand out, or one that's already connected from somewhere else, gets result 1, "Invalid Credentials".
4. Otherwise result 0, "Welcome to the world.", and the server knows the player by the address and port their packets come from.  The token isn't sent again.

UDP can lose any packet, the Connect and its answer included.  So the client sends the Connect again every half second until it gets a ConnectResult, and gives up after 10 seconds.  A repeat Connect from the address that's already connected gets result 0 again.
...
The token is how the UDP side knows who a Connect is from.  It is made when the player picks a character, one per account, and a new one replaces the old.  It lives in the server's memory and nowhere else: it dies when the TCP connection closes, and every one of them dies on a restart.  The client takes it to the UDP port in a Connect.
...
Client to server, over UDP.  The first UDP packet a client sends.  Two strings: the client's version, then the token from the WorldTicket.  Version 0.0.1:

```text
30  05 00 00 00  30 2E 30 2E 31  40 00 00 00  (the 64 characters of the token)
```

That is 78 bytes, with no length in front: the type, 4 + 5 for the version, and 4 + 64 for the token.  A token that isn't exactly 64 characters gets no answer at all.

### 0x31 ConnectResult

Server to client, over UDP, in answer to a Connect.  A `u8` result, then a string for the player.  No length in front.

| Result | Meaning | Message |
|---|---|---|
| 0 | connected | "Welcome to the world." |
| 1 | refused | "Invalid Credentials" |
| 2 | outdated client | "Outdated Client Failure" |

Connected:

```text
31  00  15 00 00 00  57 65 6C 63 6F 6D 65 20 74 6F 20 74 68 65 20 77 6F 72 6C 64 2E
```

An outdated client:

```text
31  02  17 00 00 00  4F 75 74 64 61 74 65 64 20 43 6C 69 65 6E 74 20 46 61 69 6C 75 72 65
```

That is 29 bytes: the type, the result, 4 for the string's length, and 23 for "Outdated Client Failure".# Stratum -- The Protocol
